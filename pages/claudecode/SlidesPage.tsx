...
function SlideRenderer({
  ...
  switch (slide.type) {
    ...
    case 'flowchart':
      return <FlowchartSlideView slide={slide} />;
...
function FlowchartSlideView({ slide }: { slide: FlowchartSlide }) {
  const NODE_SLOT = 200;
  const NW = 148;
  const NH = 80;
  const NRX = 12;
      ...
      <div className="mt-6 flex w-full flex-1 items-center justify-center">
        <svg viewBox={`0 0 ${VBW} ${VBH}`} className="w-full" aria-hidden>
          ...
          {slide.connections.map((conn, ci) => {
            ...
            // Cross-row: row 1 → row 2 (wrap connector)
            if (fr !== tr) {
            ...
            // Same-row loopback (going left)
            if (tx < fx) {
              if (fr === 2) {
                // Row 2: loop below the row
                const sy = bEdge(fi, fs);
                ...
                <div key={i} className="flex flex-1 flex-col items-center">
                  {node.shape === 'diamond' ? (
                    <div className="relative flex h-24 w-24 items-center justify-center md:h-32 md:w-32">
                      <div
                        className="absolute h-16 w-16 border-2 bg-white shadow-md md:h-20 md:w-20"
                        style={{ borderColor: accent, transform: 'rotate(45deg)' }}
                      />
                      <span className="relative z-10 text-4xl md:text-5xl">{node.icon}</span>
                    </div>
                  ) : (
                    <div
